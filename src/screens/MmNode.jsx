...
// MapMode と同じイベント名（カスタムイベント方式）
const FOCUS_EVENT = "mm-focus-node";

/**
 * MmNode 表示ルール：
 * ホバー時     → 折りたたみボタン
 * 選択時       → フォーマットツールバー（上部）+ クイックアクション
 *
 * 自動フォーカス：
 * forceEdit prop ではなく window カスタムイベント方式を採用。
 * react-flow が node.data をキャッシュすることで forceEdit が発火しない問題を回避。
 *
 * Enterキー動作：
 * 編集中 Enter → テキスト確定（選択状態に戻る）
 * 選択中 Enter → MapMode 側が兄弟ノード追加を処理
 */
export default function MmNode({ data, selected }) {
  ...
  useEffect(() => {
    ...
    function handleFocusEvent(e) {
      if (e.detail?.nodeId === nodeId && !editing) {
        setEditing(true);
        data.onEditStart?.();
      }
    }
  ...
  function handleKeyDown(e) {
    ...
    // Shift+Enter = 改行（textareaのデフォルト動作をそのまま使う）
    if (e.key === "Escape") {
      e.stopPropagation();
      setDraft(data.label ?? "");
      setEditing(false);
  ...
  return (
    <div onDoubleClick={startEdit} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ position: "relative", display: "inline-flex", alignItems: "center", gap: 4, padding: hasBg || isDropTarget ? "4px 8px" : "2px 4px", borderRadius: 6, background: nodeBg, border: nodeBorder, cursor: "default", userSelect: "none", maxWidth: editing ? 400 : 260, boxShadow: isDropTarget ? "0 0 0 3px #10b98140" : "none", transition: "border 0.1s" }}>
      {isDropTarget && <DropLabel />}
      <Handle id="tl" type="target" position={Position.Left}  style={{ opacity: 0, pointerEvents: "none" }} />
      <Handle id="tr" type="target" position={Position.Right} style={{ opacity: 0, pointerEvents: "none" }} />
      <Handle id="sr" type="source" position={Position.Right}
        style={!isLeft && data.hasChildren ? { background: "#fff", border: `2px solid ${PURPLE}`, width: 9, height: 9, right: -4.5, opacity: 1, pointerEvents: "none" } : { opacity: 0, pointerEvents: "none" }} />
      <Handle id="sl" type="source" position={Position.Left}
        style={isLeft && data.hasChildren ? { background: "#fff", border: `2px solid ${PURPLE}`, width: 9, height: 9, left: -4.5, opacity: 1, pointerEvents: "none" } : { opacity: 0, pointerEvents: "none" }} />

...
export default function MmNode({ data, selected }) {
  ...
  if (data.isRoot) {
    ...
    return (
      <div onDoubleClick={startEdit}
        style={{ background: bg, border: `1.5px solid ${selected ? PURPLE : "#e2e8f0"}`, borderRadius: 10, padding: "10px 18px", minWidth: 80, maxWidth: editing ? 480 : 320, boxShadow: "0 1px 4px rgba(0,0,0,0.08)", cursor: "default", userSelect: "none", position: "relative" }}>
        ...
        {showActions && (
          <>
            <QuickBtn posStyle={{ right: -36, top: "50%", transform: "translateY(-50%)" }} title="子ノードを追加 (Tab)" onClick={e => { e.stopPropagation(); data.onAddChild?.(); }}>→</QuickBtn>
            <QuickBtn posStyle={{ right: -66, top: "50%", transform: "translateY(-50%)" }} title="テンプレート挿入" onClick={e => { e.stopPropagation(); data.onInsertTemplate?.(); }} color={TEAL}>⊕</QuickBtn>
          </>
        )}
      </div>
    );
  ...
  const collapseStyle = isLeft
    ? { position: "absolute", left: -26, top: "50%", transform: "translateY(-50%)", background: "white", border: `1.5px solid ${PURPLE}`, borderRadius: "50%", width: 18, height: 18, fontSize: 7, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", padding: 0, color: PURPLE }
    : { position: "absolute", right: -26, top: "50%", transform: "translateY(-50%)", background: "white", border: `1.5px solid ${PURPLE}`, borderRadius: "50%", width: 18, height: 18, fontSize: 7, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", padding: 0, color: PURPLE };

  return (
    <div onDoubleClick={startEdit}
      style={{ position: "relative", display: "inline-flex", alignItems: "center", gap: 4, padding: hasBg ? "4px 8px" : "2px 4px", borderRadius: 6, background: hasBg ? data.nodeColor : (selected ? "rgba(168,85,247,0.07)" : "transparent"), border: `1.5px solid ${selected ? "rgba(168,85,247,0.4)" : "transparent"}`, cursor: "default", userSelect: "none", maxWidth: editing ? 400 : 260 }}>
      <Handle id="tl" type="target" position={Position.Left}  style={{ opacity: 0, pointerEvents: "none" }} />
      <Handle id="tr" type="target" position={Position.Right} style={{ opacity: 0, pointerEvents: "none" }} />
      ...
      {showActions && data.hasChildren && (
        <button title={data.collapsed ? "展開 (Cmd+/)" : "折りたたむ (Cmd+/)"} onClick={e => { e.stopPropagation(); data.onToggleCollapse?.(); }} style={collapseStyle}>
      ...
      {/* マップリンクアイコン：常時表示（リンクがある場合）クリックで遷移 */}
      {hasLink && (
        <span
          title="リンク先マップを開く"
          onClick={e => { e.stopPropagation(); data.onNavigateLink?.(); }}
          style={{ fontSize: 12, cursor: "pointer", color: TEAL, flexShrink: 0, lineHeight: 1, padding: "0 2px" }}
        >🔗</span>
      )}
      ...
      {showActions && (
        <>
          {/* 子追加 */}
          <QuickBtn posStyle={isLeft ? { left: data.hasChildren ? -50 : -36, top: "50%", transform: "translateY(-50%)" } : { right: data.hasChildren ? -50 : -36, top: "50%", transform: "translateY(-50%)" }}
            title="子ノードを追加 (Tab)" onClick={e => { e.stopPropagation(); data.onAddChild?.(); }}>
            {isLeft ? "←" : "→"}
          </QuickBtn>
          {/* 上に兄弟追加 */}
          <QuickBtn posStyle={{ top: -30, left: "50%", transform: "translateX(-50%)" }} title="上に兄弟を追加 (⌘+Enter)" onClick={e => { e.stopPropagation(); data.onAddSiblingAbove?.(); }}>↑</QuickBtn>
          {/* 下に兄弟追加 */}
          <QuickBtn posStyle={{ bottom: -30, left: "50%", transform: "translateX(-50%)" }} title="下に兄弟を追加 (Enter)" onClick={e => { e.stopPropagation(); data.onAddSiblingBelow?.(); }}>↓</QuickBtn>
          {/* テンプレート挿入 */}
          <QuickBtn posStyle={isLeft ? { left: -80, top: "50%", transform: "translateY(-50%)" } : { right: -80, top: "50%", transform: "translateY(-50%)" }}
            title="テンプレートを挿入" onClick={e => { e.stopPropagation(); data.onInsertTemplate?.(); }} color={TEAL}>⊕</QuickBtn>
          {/* マップリンク */}
          <QuickBtn posStyle={{ bottom: -56, left: "50%", transform: "translateX(-50%)" }}
            title={hasLink ? "マップリンクを変更/解除" : "マップリンクを設定"}
            onClick={e => { e.stopPropagation(); data.onMapLink?.(); }} color={hasLink ? TEAL : GRAY}>🔗</QuickBtn>
        </>
      )}
    </div>
  );
...
function QuickBtn({ children, title, onClick, posStyle, color }) {
  const [h, setH] = useState(false);
  const c = color ?? GRAY;
  return <button title={title} onClick={onClick} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
    style={{ position: "absolute", ...posStyle, background: "white", border: `1.5px solid ${h ? c : GRAY}`, borderRadius: "50%", width: 24, height: 24, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 11, color: h ? c : "#9ca3af", padding: 0, boxShadow: "0 1px 4px rgba(0,0,0,0.12)", zIndex: 10 }}>{children}</button>;
}

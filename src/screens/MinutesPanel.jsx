...
const FORMAT_MODES = [
  {
    ...
    label: "議事録",
    desc: "概要・話題・アクション",
    system: `あなたは議事録整形のアシスタントです。文字起こしテキストを読みやすい議事録に整形してください。
...
言い淀み・繰り返しは除去。専門用語はそのまま。Markdown形式で出力。`,
  },
  {
    key: "seminar",
    label: "セミナーノート",
    desc: "学び・フレームワーク整理",
    system: `あなたはセミナーノート整形のアシスタントです。文字起こしテキストをセミナーノートとして整形してください。
...
Markdown形式で出力。`,
  },
  {
    ...
    label: "アイデア整理",
    desc: "アイデアをグループ化",
    system: `あなたはアイデア整理のアシスタントです。文字起こしテキストからアイデアを抽出・整理してください。
...
Markdown形式で出力。`,
  },
  {
    ...
    label: "TODO抽出",
    desc: "タスク・決定事項のみ",
    system: `あなたはTODO・決定事項の抽出アシスタントです。文字起こしから行動すべきことと決定事項だけを抽出してください。
  ...
  return (
    <div style={{ fontSize: 13, lineHeight: 1.7, color: "#374151" }}>
      {text.split("\n").map((line, i) => {
        ...
        if (l.startsWith("## "))  return <div key={i} style={{ fontSize: 13, fontWeight: 700, color: "#1e293b", borderBottom: `1px solid ${BORDER}`, paddingBottom: 3, margin: "14px 0 5px" }}>{l.slice(3)}</div>;
        if (l.startsWith("### ")) return <div key={i} style={{ fontSize: 12, fontWeight: 700, color: "#374151", margin: "10px 0 3px" }}>{l.slice(4)}</div>;
        if (l.startsWith("> "))   return <div key={i} style={{ borderLeft: `3px solid #e2e8f0`, paddingLeft: 8, color: "#64748b", fontStyle: "italic", margin: "4px 0" }}>{l.slice(2)}</div>;
        ...
        if (l.startsWith("**") && l.includes("："))
          return <div key={i} style={{ fontWeight: 600, color: "#1e293b", marginBottom: 2 }}>{l.replace(/\*\*/g, "")}</div>;
...
export default function MinutesPanel({ open, onClose, uid }) {
  ...
  const [formatMode,  setFormatMode]  = useState("minutes");
  ...
  function startRecording() {
    ...
    if (!SR) { showToast("Chrome で開いてください", "error"); return; }
    ...
    rec.onresult = (e) => {
      ...
      for (let i = e.resultIndex; i < e.results.length; i++) {
        ...
        if (e.results[i].isFinal) fin += t; else im += t;
      }
      if (fin) {
  ...
  // ─── 整形（/api/claude プロキシ経由）────────────────────────

  async function formatMinutes() {
    const text = transcript.trim();
    if (!text) { showToast("文字起こしがありません", "error"); return; }

    const mode = FORMAT_MODES.find(m => m.key === formatMode) ?? FORMAT_MODES[0];
    ...
    try {
      const res = await fetch("/api/claude", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 4096,
          system: mode.system,
          messages: [{ role: "user", content: `以下の文字起こしを整形してください。\n\n${text}` }],
        }),
      });

      if (!res.ok) {
        const errText = await res.text().catch(() => "");
        console.error("整形APIエラー:", res.status, errText);
        showToast(`整形に失敗しました（HTTP ${res.status}）`, "error");
        setStatus("idle"); return;
      }

      const data = await res.json();
      const out  = data.content?.find(b => b.type === "text")?.text ?? "";
      if (!out) { showToast("APIレスポンスが空でした", "error"); setStatus("idle"); return; }

      setMinutes(out);
      setStatus("done");
      setTab("minutes");
    } catch (e) {
      console.error("整形エラー:", e);
  ...
  return (
    <div style={{
      ...
      background: "white", borderLeft: `1px solid ${BORDER}`,
      ...
      <div style={{ padding: "10px 14px", borderBottom: `1px solid ${BORDER}`, display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
        <div style={{ fontSize: 13, fontWeight: 700, color: "#1e293b", flex: 1 }}>録音・整形</div>
      ...
      <div style={{ padding: "12px 14px", borderBottom: `1px solid ${BORDER}`, display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
        <button onClick={isRecording ? stopRecording : startRecording} disabled={isProcessing}
          style={{
            ...
            color: isRecording ? "#ef4444" : "white", fontSize: isRecording ? 16 : 18, flexShrink: 0,
        ...
        <div style={{ flex: 1 }}>
          ...
          {isRecording
            ? <div style={{ fontSize: 20, fontWeight: 700, color: "#1e293b", fontVariantNumeric: "tabular-nums", letterSpacing: 1 }}>{formatTime(elapsed)}</div>
      ...
      <div style={{ padding: "10px 14px", borderBottom: `1px solid ${BORDER}`, flexShrink: 0 }}>
        <div style={{ fontSize: 11, color: MUTED, marginBottom: 6 }}>整形モード</div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5 }}>
          {FORMAT_MODES.map(m => (
            <button key={m.key} onClick={() => setFormatMode(m.key)}
              style={{
                background: formatMode === m.key ? "rgba(168,85,247,0.08)" : "none",
                border: `1.5px solid ${formatMode === m.key ? PURPLE : BORDER}`,
                borderRadius: 7, padding: "6px 8px", cursor: "pointer", textAlign: "left",
              }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: formatMode === m.key ? PURPLE : "#374151" }}>{m.label}</div>
              <div style={{ fontSize: 10, color: MUTED, marginTop: 1 }}>{m.desc}</div>
            </button>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: "flex", borderBottom: `1px solid ${BORDER}`, flexShrink: 0 }}>
        {[{ key: "transcript", label: "文字起こし" }, { key: "minutes", label: "整形結果" }].map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            style={{ flex: 1, padding: "7px 0", fontSize: 12, fontWeight: tab === t.key ? 700 : 400, color: tab === t.key ? PURPLE : MUTED, background: "none", border: "none", borderBottom: `2px solid ${tab === t.key ? PURPLE : "transparent"}`, cursor: "pointer" }}>
            {t.label}
          </button>
        ))}
      </div>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            <div style={{ flex: 1, position: "relative", overflow: "hidden" }}>
              <textarea ref={textareaRef}
                ...
                placeholder={isRecording ? "話してください..." : "録音を開始するとここに文字が表示されます"}
                style={{ width: "100%", height: "100%", padding: 14, boxSizing: "border-box", border: "none", outline: "none", resize: "none", fontSize: 13, lineHeight: 1.8, color: "#374151", fontFamily: "inherit", background: "transparent" }}
              />
              {isRecording && interim && (
            ...
            {transcript && (
              <div style={{ padding: "8px 12px", borderTop: `1px solid ${BORDER}`, display: "flex", gap: 7, flexShrink: 0 }}>
                ...
                <button onClick={formatMinutes} disabled={isProcessing}
                  style={{ flex: 2, background: PURPLE, color: "white", border: "none", borderRadius: 6, padding: "5px 0", fontSize: 11, fontWeight: 600, cursor: isProcessing ? "default" : "pointer", opacity: isProcessing ? 0.6 : 1 }}>
                  {isProcessing ? "整形中..." : `${FORMAT_MODES.find(m=>m.key===formatMode)?.label}に整形`}
                </button>
              </div>
            )}
          </div>
        ...
        {tab === "minutes" && (
          <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
            {!minutes && !isProcessing && (
              <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", color: MUTED, fontSize: 13, textAlign: "center", padding: 20, lineHeight: 1.8 }}>
                文字起こしタブで<br />整形ボタンを押してください
              </div>
            )}
            {isProcessing && (
            ...
            {minutes && !isProcessing && (
              <>
                ...
                <div style={{ padding: "8px 12px", borderTop: `1px solid ${BORDER}`, display: "flex", gap: 7, flexShrink: 0 }}>
                  ...
                  <button onClick={formatMinutes} disabled={isProcessing} style={{ flex: 1, background: "none", border: `1px solid ${BORDER}`, borderRadius: 6, padding: "5px 0", fontSize: 11, color: "#374151", cursor: "pointer" }}>再整形</button>

...
export default function ListMode({ uid, mapId, nodes, onNodesChange, onSaved }) {
  ...
  function handleKeyDown(e, flatNode) {
    ...
    if (e.key === "Enter" && !e.shiftKey) {
      e.preventDefault(); doEnter(flatNode, idx);
    } else if (e.key === "Tab" && !e.shiftKey) {
      e.preventDefault(); doIndent(flatNode, idx);
    } else if (e.key === "Tab" && e.shiftKey) {
      e.preventDefault(); doOutdent(flatNode);
    } else if (e.key === "Backspace" && flatNode.content === "") {
      e.preventDefault(); doDelete(flatNode, idx);
    } else if (e.key === "ArrowUp" && idx > 0) {
      e.preventDefault(); inputRefs.current[flatNodes[idx - 1].id]?.focus();
  ...
  async function doOutdent(flatNode) {
    ...
    onNodesChange(
      nodes.map(n =>
        n.id === nodeId
          ? { ...n, parent_id: grandParentId, order_index: newOrder, x: null, y: null }
          : n
      )
    );
    onSaved();
  ...
  async function doDelete(flatNode, idx) {
    ...
    onNodesChange(nodes.filter(n => n.id !== flatNode.id));
    onSaved();
...
function NodeRow({ node, inputRef, onChange, onKeyDown }) {
  ...
  return (
    <div style={{
      ...
      paddingLeft: node.depth * INDENT_PX, marginBottom: 4,
    }}>
      <span style={{
        color: T.muted, fontSize: 18, lineHeight: "26px",
      ...
      <textarea
        ...
        style={{
          ...
          color: T.fg, fontSize: 14, lineHeight: "26px",
